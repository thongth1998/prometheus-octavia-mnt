...
class PrometheusProxy(SimpleHTTPRequestHandler):
    ...
    def _add_disk_usage(self, metrics_buffer, disk_path="/"):
        disk_usage = psutil.disk_usage(disk_path)
        metrics_buffer += f"# HELP octavia_loadbalancer_disk Disk usage for {disk_path}\n"
        metrics_buffer += f"octavia_loadbalancer_disk_total {disk_usage.total}\n"
        metrics_buffer += f"octavia_loadbalancer_disk_used {disk_usage.used}\n"
        metrics_buffer += f"octavia_loadbalancer_disk_free {disk_usage.free}\n"
        metrics_buffer += f"octavia_loadbalancer_disk_percent {disk_usage.percent}\n"
    ...
    def do_GET(self):
        ...
        metrics_buffer = self._add_disk_usage(metrics_buffer)
...
def main():
    ...
    while not EXIT_EVENT.is_set():
        ...
        # it gracefully.
        try:
            httpd = ThreadingHTTPServer(('0.0.0.0', 9103),
                                        PrometheusProxy)
            ...
            print("Now serving on port 9103")
            httpd.serve_forever()
        except Exception:
            time.sleep(1)
